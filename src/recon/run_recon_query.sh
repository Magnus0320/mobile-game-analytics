#!/usr/bin/env bash
# The query runner for the Parts 1 and 2 recon pass.
#
# One query, end to end, under ARCHITECTURE.md §10.1:
#   dry run -> pre-flight gate on the estimate -> execute -> read the ACTUAL
#   bytes billed from job statistics -> record both figures and their divergence
#   -> accumulate actuals into the running total -> evaluate the halt rule.
#
# Executes through the bq CLI. No Python BigQuery client is installed: the
# project venv and both requirements files describe the environment Part 3's
# lock file certifies (§7.5), and a completed, committed part must keep
# reproducing from its own recorded environment.
#
# usage: src/recon/run_recon_query.sh <sql-file> <output-basename> <shard-range-label>
#   e.g. src/recon/run_recon_query.sh sql/02_recon_event_vocabulary.sql \
#          recon_02_event_vocabulary "20180612-20181003 (114 shards)"
#
...
# with "--" comment lines, and bq's flag parser reads a leading "--" as a
# flag name rather than as query text.
QUERY_FILE_REL="${SQL_FILE#"${REPO_ROOT}/"}"
JOB_ID="recon_$(basename "${OUT_BASE}")_$(date -u +%Y%m%dT%H%M%SZ)"
WORK="$(mktemp -d)"
...
if [[ ! "${ESTIMATE}" =~ ^[0-9]+$ ]]; then
  # Some bq versions report the dry run only as prose on stderr.
...
echo "    remaining budget : $(human "$(ledger_remaining)")"

# --- 2. the pre-flight gate.
...
# --- 3. execute. Cache is disabled because a cache hit bills zero, which would
#        make the divergence meaningless and item 12's unit cost fictitious.
#        --max_rows is explicit because bq query truncates at 100 rows by
#        default and would silently return a partial result.
...
if [[ ! "${BILLED}" =~ ^[0-9]+$ ]]; then
  ...
  echo "A-085 makes the running total untrackable in actuals without it; this" >&2
...
# --- 5. publish the result and its provenance sidecar (A-086).
#        The CSV stays bare-header so it loads with a plain read_csv, exactly
#        like Part 3's tables; the five provenance fields travel beside it.
...
(out, query_file, job_id, job_date, shard_range,
 estimate, processed, billed, row_count, csv_name) = sys.argv[1:11]
estimate, processed, billed = int(estimate), int(processed), int(billed)
meta = {
...
echo "    running total    : $(human "$(ledger_total_billed)") of 200 GiB"

if ! halt_check "${ESTIMATE}" "${BILLED}"; then
  echo "    The pass stops here. Record the divergence and what it implies about" >&2
