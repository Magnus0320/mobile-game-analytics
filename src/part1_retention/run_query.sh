#!/usr/bin/env bash
# The query runner for the Part 1 build session.
#
...
#   the halt rule.
#
# Executes through the bq CLI (A-087's pattern, A-131's decision). No BigQuery
# Python client is installed: .venv, requirements.txt and requirements.lock.txt
# describe the environment Part 3's clean-checkout guarantee rests on (§7.5),
# and a completed, committed part must keep reproducing from it.
#
# usage: src/part1_retention/run_query.sh <sql-file> <output-basename> <shard-range-label>
#   e.g. src/part1_retention/run_query.sh sql/13_part1_classic_retention.sql \
#          part1_q13_classic_retention "20180612-20181003 (114 shards)"
#
...
JOB_ID="part1_$(basename "${OUT_BASE}")_$(date -u +%Y%m%dT%H%M%SZ)_$$"
WORK="$(mktemp -d)"
...
# --- 3. execute. Cache is disabled (A-132): a cache hit bills zero, which would
...
#        The CSV stays bare-header so it loads with a plain reader, exactly like
#        Part 3's tables; the provenance fields travel beside it.
...
echo "    session total    : $(human "$(ledger_total_billed)") of 20 GiB; pair total against 40 GiB"
